...
Capstone Project: Jenkins → Terraform → EKS (with ECR and Docker)
...
Jenkins setup
Create two Global “Secret text” credentials:
ID: AWS_ACCESS_KEY_ID → value: your AWS access key ID
ID: AWS_SECRET_ACCESS_KEY → value: your AWS secret access key
Create a Pipeline job pointing to this repo (Pipeline script from SCM)
Ensure the job uses Jenkinsfile (default path)
Agent requires: awscli, terraform, kubectl, docker in PATH
Pipeline parameters
AWS_REGION (default: ap-south-1)
ECR_REPO (default: gl-capstone-project-pan-repo)
CLUSTER_NAME (default: capstone-project-eks-cluster)
How to run
Run the Jenkins job with defaults
Stages executed:
Checkout
...
After success, get Service External IP:
